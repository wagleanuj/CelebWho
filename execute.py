...
if __name__ == '__main__':
    ...
    eigenmodel.load('eigenmodel.yaml')
    testfiles='tests'
    ...
    try:
        pre_image = cv2.imread(given, cv2.IMREAD_GRAYSCALE)
    except:
    ...
    for bBox in bBoxes:
        ...
        cv2.imwrite("testobj.jpg",crop_gray_frame)#for displaying the original picture
        n = []
        flag = 1

        predicted_label = eigenmodel.predict(np.asarray(crop_gray_frame))
        print("This person in the image ' " + given + "' looks like" + ":" + people[
            predicted_label[0]] + " Confidence level: " + str(predicted_label[1]))
        filename = (people[predicted_label[0]])
        newpath = os.path.join("allactors", filename)
        g = listdir(newpath)[0]
        ss = os.path.join(newpath, g)
        img = cv2.imread(ss, cv2.IMREAD_GRAYSCALE)
        cv2.imwrite("testoutput.jpg",img)
        imgorig=Image.open("testobj.jpg")
        imgorig.show()
        imggoutput = Image.open("testoutput.jpg")
        imggoutput.show()
